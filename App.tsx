...
import { Point, AnalysisResult, GameMetadata, AppSettings, GameTab } from './types';
...
const App: React.FC = () => {
  ...
  const [tabs, setTabs] = useState<GameTab[]>([
      {
          id: 'default-1',
          title: '新局 1',
          rootNode: { ...rootNode, id: 'root-default' },
          currentNodeId: 'root-default',
  ...
  const [metadata, setMetadata] = useState<GameMetadata>(DEFAULT_METADATA);
  ...
  }, [rootNode, currentNode.id, metadata]); // Dependency array ensures update on change
  ...
  const handleSwitchTab = (targetId: string) => {
      ...
      if (targetTab) {
          ...
          // Restore State
          setRootNode(targetTab.rootNode);
          // Note: jumpToMove relies on the NEW rootNode which is set asynchronously in React.
          // However, setRootNode in hook updates state. We need to queue the jump.
          // A safer way is to rely on `useEffect` in hook or pass data directly if hook allows.
          // Here we assume standard React batching. To be safe, we use a timeout or verify rootNode.
          // Actually, our hook exposes setRootNode which updates 'rootNode' state.
          // We also need to update currentNodeId *after* rootNode is set.
          // BUT, `useMoveTree` manages both.
          // Let's modify `useMoveTree` to allow setting both at once or trust `importGame` equivalent?
          // Actually `setRootNode` in our hook also sets `currentNodeId` to the node's ID.
          // We need a way to restore to a specific node ID.
          
          // Let's use a workaround: The `setRootNode` helper in hook sets current to root.
          // We need to set current to `targetTab.currentNodeId`.
          // We can call `jumpToMove` but we need the node object.
          // Helper to find node in the tree structure (which is just data)
          const findNode = (id: string, start: any): any => {
          ...
          const targetNode = findNode(targetTab.currentNodeId, targetTab.rootNode);
          ...
          // Then Jump (Defer slightly to allow render cycle if needed, or if setRootNode is sync enough)
          setTimeout(() => {
              if (targetNode) {
                  jumpToMove(targetNode);
              }
          }, 0);
      }
  };
  ...
  return (
    <>
      ...
      <InfoModal isOpen={showInfoModal} onClose={() => setShowInfoModal(false)} metadata={metadata} onSave={(m) => { setMetadata(m); setTabs(prev => prev.map(t => t.id === activeTabId ? {...t, metadata: m, title: m.title || t.title} : t)); }} />
