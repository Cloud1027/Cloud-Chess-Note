            ...
            <div className="flex items-center gap-1 p-2">
                ...
                >
                    <div className="flex items-center gap-2 overflow-hidden">
                        <div className={`w-2 h-2 rounded-full ${getColorClass(activeTab.colorTag)}`} />
                        <span className="font-bold text-zinc-200 text-sm truncate max-w-[140px]">
                            {activeTab.title || '未命名棋譜'}
                        </span>
            ...
            {isOpen ? (
                ...
                <div className="flex items-center justify-between px-2 pb-2 text-[10px] text-zinc-500">
                    ...
                    <div className="flex gap-1">
                        <button onClick={() => startEditing(activeTab, { stopPropagation: () => {} } as any)} className="hover:text-zinc-300">命名</button>
                        <span className="opacity-50">|</span>
                        <button onClick={() => onDelete(activeTabId)} className="hover:text-red-400 disabled:opacity-50" disabled={tabs.length <= 1}>刪除</button>
                    </div>
                </div>
            )}
        </div>
